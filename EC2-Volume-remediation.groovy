...
properties([parameters([string(defaultValue: 'Develop', description: 'Enter the git branch name', name: 'branch', trim: false), string(defaultValue: '', description: 'Enter account number', name: 'account', trim: true), string(defaultValue: 'us-east-1', description: 'enter region', name: 'region', trim: false), string(defaultValue: '', description: 'instance ID whose volume to RE ENCRYPT', name: 'instance_id', trim: false), string(defaultValue: '', description: 'The value of  the new CMK Alias to use it. If one was not found a new one will be created', name: 'new_kms_key_alias', trim: true), booleanParam(defaultValue: false, description: 'When checked this automation will delete the original volume', name: 'delete_original_volumes'), booleanParam(defaultValue: false, description: 'When checked, this automation will restart the EC2 instance', name: 'restart_instance'), booleanParam(defaultValue: true, description: 'Set to true to display results of the remediation plan without taking any action', name: 'preview')])])
node ('CPE') {
    stage('SCM') {
        //set up proxy
        ...
        deleteDir()
          ...
          sh '''
            ...
            then
                ...
                python remediator.py -command ${command} -scope ${scope} -region ${region} -preview
            else
                echo "running in remediation mode"
                python remediator.py -command ${command} -scope ${scope} -region ${region}
            fi
          '''
    }
